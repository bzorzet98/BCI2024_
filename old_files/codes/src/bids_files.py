...
def save_raw_bids(data, exg_channels, markers_dict, mne_info, subject_information, save_data_path, experiment_information, session_type): 
    """
    ...
    # Dominance
    dominance = subject_information['dominance']
    if (dominance == 'right'):
        domi = 1
    elif (dominance == 'left'):
        domi = 2
    ...
    bids_path = BIDSPath(subject=subject_information['subject_ID'],
                         session=experiment_information['session_ID'],
                         task=session_type,
                         run=experiment_information['run_ID'],
                         root=os.path.join(save_data_path, experiment_information['project_name']))
