...
while True:   # Until the end_game marker is received
    ...
    if received_data: # if NEW data has been received since last ReadReceivedData function call
        ...
        if marker_code != markers_dict['end_game']:
            ...
            elif marker_code in [markers_dict['go_cue_left'], markers_dict['go_cue_right'], markers_dict['go_cue_up'], markers_dict['go_cue_down']]:
                trial_array, y_pred = predict_one_trial_SSVEP(board, exg_channels, mne_info, trained_pipeline)
                # left=8.5, right=10, up=12, down=15
                if marker_code == markers_dict['go_cue_left']:
                    y_true = 1
                elif marker_code == markers_dict['go_cue_right']:
                    y_true = 2
                elif marker_code == markers_dict['go_cue_up']:
                    y_true = 3
                elif marker_code == markers_dict['go_cue_down']:
                    y_true = 4

                # This line is to send a marker to the Unity application
                # Acá hay que ver qué recibe en Unity. Tal vez es mejor cambiar los valores de y_pred por otro string que no sean los numeros
                sock.SendData(str(y_pred))
