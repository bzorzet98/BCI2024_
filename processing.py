...
def find_corr(n_components, eeg_data, freq):
    ...
    for freq_idx in range((freq.shape)[0]):
        ...
        cca.fit(eeg_data.T, np.squeeze(freq[freq_idx]).T)
        # Transform the EEG data and reference signals into the canonical space
        O1_a, O1_b = cca.transform(eeg_data.T, np.squeeze(freq[freq_idx]).T)
...
def predict_one_trial_SSVEP(board, exg_channels_indices, mne_info):
    ...
    trial_epoch.pick(['O1', 'O2'])
    # Bandpass filter from 1 to 40 Hz
    trial_epoch.filter(1, 40)
    ...
    freq = np.array([generate_reference_signals(f, sampling_rate, trial_array.shape[1], n_harmonics) for f in frequencies])
    # Compute CCA 
